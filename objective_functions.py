...
def MNIST_plus_train_loss(param, no_stop=False, incumbent=None, bo_iteration=0, stds=[], N=50, N_init_epochs=8):
    '''
        param: parameters
    ...
    batch_size_ = param[0]
    batch_size = int(
        batch_size_ * (parameter_range[0][1] - parameter_range[0][0]) + parameter_range[0][0])
    C_ = param[1]
    ...
    print(
        "[Evaluating parameters: batch size={0}/C={1}/lr={2}]".format(batch_size, C, learning_rate))

    # The tensorflow model of logistic regression is built below

    # tf Graph Input
    # mnist data image of shape 28*28=784
    x = tf.placeholder(tf.float32, [None, n_ft])
    ...
    cost = tf.reduce_mean(-tf.reduce_sum(y*tf.log(pred),
                                         reduction_indices=1) + C * regularizers)
    # Gradient Descent
    ...
    neg_log_loss = tf.reduce_mean(-tf.reduce_sum(y *
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in tqdm(range(training_epochs)):
            ...
            for i in range(total_batch):
                batch_xs, batch_ys = X_train[(
                    i*batch_size):((i+1)*batch_size), :], Y_train[(i*batch_size):((i+1)*batch_size), :]
            ...
            if (epoch+1 == num_init_curve) and (not no_stop):
                ...
                action_regions, grid_St = run_BOS_plus(
                    1-np.array(val_epochs), train_info, incumbent, training_epochs, bo_iteration)
                time_BOS = time.time() - time_start

            # start using the decision rules obtained from BOS
            if (epoch >= num_init_curve) and (not no_stop):
                state = np.sum(
                    1 - np.array(val_epochs[num_init_curve:])) / (epoch - num_init_curve + 1)
                ind_state = np.max(np.nonzero(state > grid_St)[0])
                action_to_take = action_regions[epoch -
                                                num_init_curve, ind_state]
...
def CIFAR_10_plus_train_loss(param, no_stop=False, incumbent=None, bo_iteration=0, stds=[], N=50, N_init_epochs=8):
    '''
        param: parameters
        no_stop: if TRUE, then the function evaluation never early-stops
        incumbent: the currently found maximum value
    ...
    # transform the input to the real range of the hyper-parameters, to be used for model training
    parameter_range = [[32, 512], [1e-7, 0.1],
                       [1e-7, 1e-3], [1e-7, 1e-3], [128, 256], [256, 512]]
    batch_size_ = param[0]
    batch_size = int(
        batch_size_ * (parameter_range[0][1] - parameter_range[0][0]) + parameter_range[0][0])
    ...
    learning_rate_decay = learning_rate_decay_ * \
        (parameter_range[2][1] - parameter_range[2][0]) + parameter_range[2][0]
    l2_regular_ = param[3]
    l2_regular = l2_regular_ * \
        (parameter_range[3][1] - parameter_range[3][0]) + parameter_range[3][0]
    conv_filters_ = param[4]
    conv_filters = int(
        conv_filters_ * (parameter_range[4][1] - parameter_range[4][0]) + parameter_range[4][0])
    dense_units_ = param[5]
    dense_units = int(
        dense_units_ * (parameter_range[5][1] - parameter_range[5][0]) + parameter_range[5][0])
    ...
    model.add(Conv2D(conv_filters, (kernel_size, kernel_size),
                     padding='same', kernel_regularizer=regularizers.l2(l2_regular)))
    model.add(Activation('relu'))
    model.add(MaxPooling2D(pool_size=(pool_size, pool_size)))
    model.add(Dropout(dropout_rate))
    ...
    for epoch in tqdm(range(training_epochs)):
        ...
        if (epoch+1 == num_init_curve) and (not no_stop):
            print("initial learning errors: ", 1 - np.array(val_epochs))
            time_start = time.time()
            # add BOBOS-PLUS specific info, AKA training accuracy. Must be a 2D array of shape <num_data x num_features>
            train_info = 1 - np.array(train_epochs).reshape(-1, 1)
            action_regions, grid_St = run_BOS_plus(
                1 - np.array(val_epochs), train_info, incumbent, training_epochs, bo_iteration)
            time_BOS = time.time() - time_start
...
#             val_log_loss = neg_log_loss.eval({x:X_test, y:Y_test})
            ...
            if (epoch+1 == num_init_curve) and (not no_stop):
                print("initial learning errors: ", 1 - np.array(val_epochs))
                time_start = time.time()
                action_regions, grid_St = run_BOS(
                    1 - np.array(val_epochs), incumbent, training_epochs, bo_iteration)
                time_BOS = time.time() - time_start
