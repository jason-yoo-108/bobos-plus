...
def parameters_different(params_old, params_new):
    zipped = zip(params_old, params_new)
    for p_next, p_current in zipped:
        if p_next.data.ne(p_current.data).sum() > 0.:
            return True
    return False
...
class Sls(torch.optim.Optimizer):
    ...
    def step(self, closure):
        ...
        for group in self.param_groups:
            ...
            grad_current_COPY = deepcopy(grad_current)
            ...
            with torch.no_grad():
                if grad_norm >= 1e-8:
                    ...
                    for e in range(100):
                        ...
                        ut.try_sgd_update(params, step_size, params_current, grad_current_COPY)
                        ...
                        if group['line_search_fn'] == "armijo":
                            ...
                            #     print('ok.')
                            #     print(e, found)
                            #     Ngrad_norm = ut.compute_grad_norm(grad_current_COPY)
                            #     Ograd_norm = ut.compute_grad_norm(grad_current)
                            #     print(Ograd_norm, Ngrad_norm)
                            #     print(loss, loss_next)
                            #     print(step_size, step_size_old)
                            #     print(group.keys())
                            if not parameters_different(params_current, params):
                                 warnings.warn('not updated.')
                                # print('post step', params)
                                # print('original', params_current)
                            if found == 1:
                                break
